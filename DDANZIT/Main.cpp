...
#pragma region Player
	...
	aSp->TryAddAtlasRect("idle", SpriteAtlasRect(0, 0, 382, 556));
	aSp->TrySetAtlas("idle");

	ariObj->transform()->SetParent(player->transform());
	ariObj->transform()->SetLocalScale(Vector2f(1.f, 1.1f));
	...
	suObj->transform()->SetLocalScale(Vector2f(0.8f, 0.8f));
	...
	wSp->TryAddAtlasRect("idle", SpriteAtlasRect(0, 0, 280.5f, 525.5f));
	wSp->TrySetAtlas("idle");

	wooObj->transform()->SetParent(player->transform());
	wooObj->transform()->SetLocalScale(Vector2f(1.27f, 1.27f));
...
int main()
{
	if (!DDANZIT_Initialize(L"Game", 1600, 900, filePath, 3,Mp3FilePath,2,Mp4FilePath,3,SfxFilePath,1))
		return -1;
